...
- **Audit log** — every LLM call and tool invocation is persisted to `audit_events` (SQLite mode only). Owner-scoped read at `GET /api/sessions/{id}/audit`. A `tool_call` row's `detail.outcome` is `OK`, `REFUSED` or `ERROR` (`detail.ok` is kept for older readers and is true only for `OK`), so a refused read never looks like a granted one — the sequence is shown for real in [docs/consent-demo.md](docs/consent-demo.md).
